...
# Add project root to sys.path to allow importing 'engine'
# This script is intended to be run from the project root as 'python scripts/test_engine_historic_qa.py'
# or discovered by unittest when 'python -m unittest discover' is run from project root,
...
# Define HISTORIC_QA_DATA_DIR relative to the project root (math_tutor/)
# Assumes this test script is in math_tutor/scripts/
HISTORIC_QA_DATA_DIR = Path(__file__).resolve().parent.parent / "historic_qa_data"
...
class TestHistoricQARetrieval(unittest.TestCase):
    ...
    @patch('engine.OpenAIEmbeddings') # Mocks the class for historic QA embeddings
    @patch('engine.ChatOpenAI')       # Mocks the class for LLM instantiations
    ...
    def test_historic_answer_found_grade11(
        ...
        mock_chat_openai_class, mock_openai_embeddings_class, 
        mock_rephrase_question, mock_get_student_level):
        ...
        mock_search_historic_qa.return_value = (self.mock_historic_answer, "mock_id_g11_found")
        ...
        mock_openai_embeddings_instance = mock_openai_embeddings_class.return_value # Ensure instance is defined in this scope if used
        ...
        mock_chat_openai_class.assert_called() 
        self.assertEqual(result, (self.mock_historic_answer, "mock_id_g11_found"))
    ...
    def test_historic_answer_found_grade12(
        self, mock_get_topic, mock_search_historic_qa, mock_load_historic_qa_resources, 
        mock_chat_openai_class, mock_openai_embeddings_class, mock_rephrase_question, mock_get_student_level):
        ...
        mock_db_conn = MagicMock()
        mock_load_historic_qa_resources.return_value = (mock_faiss_index, mock_db_conn)
        mock_search_historic_qa.return_value = (self.mock_historic_answer, "mock_id_g12_found") # This will be returned by the mock
        ...
        mock_chat_openai_class.assert_called()
        self.assertEqual(result, (self.mock_historic_answer, "mock_id_g12_found"))
    ...
    def test_historic_answer_not_found_due_to_threshold(
        ...
        mock_load_historic_qa_resources, mock_chat_openai_class, mock_openai_embeddings_class, 
        mock_rephrase_question, mock_get_student_level):
    ...
    def test_historic_qa_skipped_for_other_grades(
        self, mock_llm_chain, mock_get_relevant_chunks, mock_get_topic, mock_chat_openai_class, 
        mock_openai_embeddings_class, mock_load_historic_qa_resources, mock_rephrase_question, mock_get_student_level):
        ...
        mock_answer_chain_instance = mock_llm_chain.return_value
        mock_answer_chain_instance.run.return_value = self.mock_normal_pipeline_answer
        mock_embeddings_instance = mock_openai_embeddings_class.return_value # To check calls
        ...
        mock_embeddings_instance.embed_query.assert_not_called() # embed_query for historic Q&A part
    ...
    def test_historic_qa_configurable_threshold_allows_retrieval(
        ...
        mock_search_historic_qa.return_value = (self.mock_historic_answer, "mock_id_thresh_allow_final") 
        ...
        mock_search_historic_qa.assert_called_once_with(
            unittest.mock.ANY, mock_faiss_index, mock_db_conn, 
            k=HISTORIC_QA_K_NEIGHBORS, 
            l2_threshold=custom_loose_threshold
        )
        mock_chat_openai_class.assert_called()
        self.assertEqual(result, (self.mock_historic_answer, "mock_id_thresh_allow_final"))
        mock_get_topic.assert_not_called()
    ...
    @patch('utilities.qa_utils.sqlite3.connect') # Mock sqlite3.connect where it's used in qa_utils
    ...
    @patch('engine.ChatOpenAI')
    # engine.faiss is not directly used by load_historic_qa_resources; faiss is used within qa_utils
    @patch('utilities.qa_utils.faiss.read_index') 
    @patch('utilities.qa_utils.sqlite3.connect')
    @patch('os.path.exists')
    def test_load_historic_resources_success(self, mock_path_exists, mock_sqlite_connect, mock_faiss_read_index, mock_chat_openai_class):
        """Test load_historic_qa_resources successfully loads FAISS index and DB connection."""
        from utilities.qa_utils import load_historic_qa_resources
        # HISTORIC_QA_DATA_DIR is now defined at the top of the file
        ...
        def side_effect_os_path_exists(path):
            ...
            return False
        mock_path_exists.side_effect = side_effect_os_path_exists
        ...
        mock_sqlite_connect.return_value = mock_db_conn_instance
        ...
        mock_path_exists.assert_any_call(expected_index_path)
        mock_path_exists.assert_any_call(expected_db_path)
        mock_faiss_read_index.assert_called_once_with(expected_index_path)
    ...
    def test_load_historic_resources_index_file_not_found(self, mock_path_exists, mock_chat_openai_class):
    ...
    @patch('engine.ChatOpenAI')
    @patch('utilities.qa_utils.faiss.read_index', side_effect=Exception("FAISS load error")) # Patched in qa_utils
    @patch('os.path.exists', return_value=True) # Assume index file exists but read fails
    def test_load_historic_resources_faiss_read_fails(self, mock_path_exists, mock_faiss_read_index, mock_chat_openai_class):
        """Test load_historic_qa_resources returns (None, None) if FAISS index file not found."""
        from engine import load_historic_qa_resources
        faiss_index, db_conn = load_historic_qa_resources(11)
        self.assertIsNone(faiss_index)
    ...
    @patch('engine.ChatOpenAI')
    @patch('os.path.exists')
    def test_load_historic_resources_db_file_not_found(self, mock_path_exists, mock_chat_openai_class):
        ...
        def side_effect_os_path_exists(path):
            if path == expected_index_path:
                return True  # Index exists
            elif path == expected_db_path:
        ...
        with patch('utilities.qa_utils.faiss') as mock_faiss_module:
